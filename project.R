...
# ========== 1) Extract the first 3 PCs from the training set ==========
...
# ========== 2) Apply the same PCA transformation to the test set ==========

# Use 'predict' on pca_res to get PCs for the test data’s numeric variables
...
# ========== 3) Ensure cisa_kev is a factor with "Yes"/"No" ==========
...
# ========== 4) Fit a logistic regression model on the PCA + categorical data ==========
...
# Create a recipe that performs dummy encoding on categorical predictors and applies SMOTE
rec <- recipe(cisa_kev ~ ., data = train_data_pca) %>%
  step_dummy(all_nominal_predictors()) %>%  # convert categorical predictors to dummies
  step_smote(cisa_kev)                      # apply SMOTE to balance the target variable

# Define train control without the sampling argument (since it's now in the recipe)
...
# Train a logistic regression model using the recipe
...
table(train_data_clean$cisa_kev)
